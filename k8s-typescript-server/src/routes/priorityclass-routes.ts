  ...
  router.patch('/apis/scheduling.k8s.io/v1/priorityclasses/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('priorityclass', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('priorityclass', name, patchData, namespace, resource.metadata.resourceVersion);
