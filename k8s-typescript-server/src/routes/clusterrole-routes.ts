...
//delete collection of ClusterRole
  router.delete('/apis/rbac.authorization.k8s.io/v1/clusterroles', async (req, res, next) => {
    try {
      ...
      const fieldSelector = req.query.fieldSelector as string | undefined;
      const namespace = null;
      logger.info(`Deleting all clusterrole ${namespace}`);
      try {

        const deleted = await storage.deleteAllResources('clusterrole', namespace, { labelSelector, fieldSelector });
        
        if (!deleted) {
          return handleResourceError(new Error(`clusterrole not found in namespace ${namespace}`), res);
        }
      } catch(e) {
          return handleResourceError(new Error(`clusterrole not deleted in namespace ${namespace}. Error: ${(e as Error).message}`), res);
      }
  ...
  router.delete('/apis/rbac.authorization.k8s.io/v1/clusterroles/:name', async (req, res, next) => {
    try {
      ...
      try {
        ...
        const deleted = await storage.deleteResource('clusterrole', name, namespace);
        
        if (!deleted) {
          return handleResourceError(new Error(`clusterrole ${name} not found in namespace ${namespace}`), res);
        }
      } catch(e) {
          return handleResourceError(new Error(`clusterrole ${name} not deleted in namespace ${namespace}. Error: ${(e as Error).message}`), res);
      }
  ...
  router.patch('/apis/rbac.authorization.k8s.io/v1/clusterroles/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('clusterrole', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('clusterrole', name, patchData, namespace, resource.metadata.resourceVersion);
