  ...
  router.patch('/apis/apps/v1/namespaces/:namespace/deployments/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('deployment', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('deployment', name, patchData, namespace, resource.metadata.resourceVersion);
