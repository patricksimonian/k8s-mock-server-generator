  ...
  router.patch('/api/v1/namespaces/:namespace/services/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = storage.mergePatchResource('service', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = storage.jsonPatchResource('service', name, patchData, namespace, resource.metadata.resourceVersion);
