...
//replace status of the specified ResourceQuota
  router.put('/api/v1/namespaces/:namespace/resourcequotas/:name/status', async (req, res, next) => {
    try {
      ...
      const updatedResource = await storage.updateSubresource('resourcequota', name, subresource, resource, namespace);
  ...
  router.patch('/api/v1/namespaces/:namespace/resourcequotas/:name/status', async (req, res, next) => {
    try {
      ...
      const updatedResource = await storage.updateSubresource('resourcequota', name, subresource, patchData, namespace);
      return res.json(updatedResource);
    } catch (error) {
      next(error);
    }
