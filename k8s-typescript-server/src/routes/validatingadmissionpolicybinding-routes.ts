  ...
  router.delete('/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicybindings/:name', async (req, res, next) => {
    try {
      ...
      try {
        ...
        const deleted = await storage.deleteResource('validatingadmissionpolicybinding', name, namespace);
        
        if (!deleted) {
          return handleResourceError(new Error(`validatingadmissionpolicybinding ${name} not found in namespace ${namespace}`), res);
        }
      } catch(e) {
          return handleResourceError(new Error(`validatingadmissionpolicybinding ${name} not deleted in namespace ${namespace}. Error: ${(e as Error).message}`), res);
      }
  ...
  router.patch('/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicybindings/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('validatingadmissionpolicybinding', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('validatingadmissionpolicybinding', name, patchData, namespace, resource.metadata.resourceVersion);
