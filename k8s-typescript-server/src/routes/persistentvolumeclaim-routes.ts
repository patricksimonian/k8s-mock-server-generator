  ...
  router.patch('/api/v1/namespaces/:namespace/persistentvolumeclaims/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = storage.mergePatchResource('persistentvolumeclaim', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = storage.jsonPatchResource('persistentvolumeclaim', name, patchData, namespace, resource.metadata.resourceVersion);
