  ...
  router.patch('/apis/admissionregistration.k8s.io/v1/validatingwebhookconfigurations/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('validatingwebhookconfiguration', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('validatingwebhookconfiguration', name, patchData, namespace, resource.metadata.resourceVersion);
...
//delete collection of ValidatingWebhookConfiguration
  router.delete('/apis/admissionregistration.k8s.io/v1/validatingwebhookconfigurations', async (req, res, next) => {
    try {
      ...
      const fieldSelector = req.query.fieldSelector as string | undefined;
      const namespace = null;
      logger.info(`Deleting all validatingwebhookconfiguration ${namespace}`);
      try {

        const deleted = await storage.deleteAllResources('validatingwebhookconfiguration', namespace, { labelSelector, fieldSelector });
        
        if (!deleted) {
          return handleResourceError(new Error(`validatingwebhookconfiguration not found in namespace ${namespace}`), res);
        }
      } catch(e) {
          return handleResourceError(new Error(`validatingwebhookconfiguration not deleted in namespace ${namespace}. Error: ${(e as Error).message}`), res);
      }
