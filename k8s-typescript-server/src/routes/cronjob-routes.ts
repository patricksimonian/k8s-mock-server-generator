  ...
  router.patch('/apis/batch/v1/namespaces/:namespace/cronjobs/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('cronjob', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('cronjob', name, patchData, namespace, resource.metadata.resourceVersion);
  ...
  router.delete('/apis/batch/v1/namespaces/:namespace/cronjobs', async (req, res, next) => {
    try {
      ...
      try {
        ...
        const deleted = await storage.deleteAllResources('cronjob', namespace, { labelSelector, fieldSelector });
