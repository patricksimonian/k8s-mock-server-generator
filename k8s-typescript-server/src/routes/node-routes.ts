  ...
  router.patch('/api/v1/nodes/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = storage.mergePatchResource('node', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = storage.jsonPatchResource('node', name, patchData, namespace, resource.metadata.resourceVersion);
