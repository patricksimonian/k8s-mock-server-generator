  ...
  router.patch('/apis/storage.k8s.io/v1/csinodes/:name', async (req, res, next) => {
    try {
      ...
      if (
        ...
        const updatedResource = await storage.mergePatchResource('csinode', name, patchData, namespace, resource.metadata.resourceVersion);
        return res.json(updatedResource);
      } else if (contentType === 'application/json-patch+json') {
        // JSON patch: apply an array of operations
        try {
          const updatedResource = await storage.jsonPatchResource('csinode', name, patchData, namespace, resource.metadata.resourceVersion);
